...
load("~/DSI_DTI_FA_ConnMatrix/updated_atlases.RData")
#remove old binary atlases from environment as a precaution
rm(connections_from_atlas)
#load myaparc36 binarised atlas
myaparc_36_updated_atlas = combined_atlas$myaparc_36_connections_from_atlas
...
control_gfa_allparc_multiplied_updated = list()
#start count
count = 1 
#start for loop 
for(i in input){
  ...
  headers_check = identical(dimnames(input_data), dimnames(myaparc_36_updated_atlas)) 
  ...
  #multiply matrices
  multiplied_matrix = input_data*myaparc_36_updated_atlas
  #parcellation for adding to list
  parcellation = "myaparc.36"
  #add to list w participant ID
  control_gfa_allparc_multiplied_updated[[parcellation]][[participant_id]] = list(gfa_connectivity = multiplied_matrix)
...
myaparc_60_updated_atlas = combined_atlas$myaparc_60_connections_from_atlas
#change input list
...
for(i in input){
  ...
  headers_check = identical(dimnames(input_data), dimnames(myaparc_60_updated_atlas)) 
  if(headers_check == "TRUE"){
    print("column do match")
  } else{
    print("column names do not match")
  ...
  #multiply matrices
  multiplied_matrix = input_data*myaparc_60_updated_atlas
  #parcellation for adding to list
...
myaparc_125_updated_atlas = combined_atlas$myaparc_125_connections_from_atlas
#change input list
...
for(i in input){
  ...
  #check columns and rows match 
  headers_check = identical(dimnames(input_data), dimnames(myaparc_125_updated_atlas)) 
  if(headers_check == "TRUE"){
    print("column do match")
  } else{
    print("column names do not match")
  ...
  #multiply matrices
  multiplied_matrix = input_data*myaparc_125_updated_atlas
  #parcellation for adding to list
...
myaparc_250_updated_atlas = combined_atlas$myaparc_250_connections_from_atlas
#change input list
...
for(i in input){
  ...
  #check columns and rows match 
  headers_check = identical(dimnames(input_data), dimnames(myaparc_250_updated_atlas)) 
  if(headers_check == "TRUE"){
    print("column do match")
  } else{
    print("column names do not match")
  ...
  #multiply matrices
  multiplied_matrix = input_data*myaparc_250_updated_atlas
  #parcellation for adding to list
...
#empty list to store the multiplied matrices
patient_gfa_allparc_multiplied_updated = list()
#start count
count = 1 
#start for loop 
for(i in input){
  ...
  parcellation = "myaparc.36"
  #add to list w participant ID
  patient_gfa_allparc_multiplied_updated[[parcellation]][[participant_id]] = list(gfa_connectivity = multiplied_matrix)
...
#### tidy up environment ####
rm(i, input, input_data, multiplied_matrix, region_names125, region_names250, region_names36, region_names60, connType, count, dsiStudio, genConn, gfa_folder, headers_check, input_file, list_of_files, loc, new_file_name, parcellation, parcScheme, participant_id)
rm(countzeros_allparc, countzerosdata, gfa_connmatrix, c, col, p, parcellations, r, row)
